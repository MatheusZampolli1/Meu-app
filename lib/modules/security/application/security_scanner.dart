...
class SecurityScanner {
  ...
  Future<SecurityScanResult> runScan() async {
    ...
    final score = (100 - riskValue).clamp(0, 100).toDouble();

    await _eventRepository.addEvent(
      type: 'scan',
      title: 'Security scan completed',
      message:
          'Detected ${risks.where((risk) => risk.granted).length} granted sensitive permissions.',
      meta: {
        'grantedPermissions': risks.where((risk) => risk.granted).length,
        'score': score,
      },
    );
