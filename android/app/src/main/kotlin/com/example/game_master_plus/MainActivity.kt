...
import com.example.game_master_plus.perf.BatteryOpt
import com.example.game_master_plus.PerfForegroundService
...
class MainActivity : FlutterActivity() {
    ...
    private val perfChannel = "fluxon/perf"
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, perfChannel)
            .setMethodCallHandler { call, result ->
                when (call.method) {
                    "startTurbo" -> {
                        val ok = try {
                            PerfForegroundService.start(this@MainActivity)
                            true
                        } catch (_: Exception) {
                            false
                        }
                    ...
                    "stopTurbo" -> {
                        val ok = try {
                            PerfForegroundService.stop(this@MainActivity)
                            true
                        } catch (_: Exception) {
                            false
                        }
                    ...
                    "requestIgnoreBatteryOpt" -> {
                        val ok = try {
                            BatteryOpt.requestIgnore(this@MainActivity)
                            true
                        } catch (_: Exception) {
                            false
                        }
